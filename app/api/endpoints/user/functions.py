...
def get_current_active_user(current_user: UserModel.User = Depends(get_current_user)):
    if current_user.is_left:
        raise HTTPException(
...
def get_current_admin_user(current_user: UserModel.User = Depends(get_current_active_user)):
    if current_user.role != UserRole.ADMIN:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only admins are allowed"
        )
    return current_user